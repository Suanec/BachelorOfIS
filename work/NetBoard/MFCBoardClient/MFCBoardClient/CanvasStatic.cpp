...
void CCanvasStatic::Draw()
{
	...
	if( m_bOwnerDraw )
	{
		...
		{
            if( WBSERVER == ( ( CMFCBoardClientDlg * )GetParent() )->IsServer )
			{
				( ( CMFCBoardClientDlg * )GetParent() )->SendDrawInfoToAllUser( m_ptStart, m_ptEnd );
			}
            else if( WBCLIENT == ( ( CMFCBoardClientDlg * )GetParent() )->IsServer )
			{
				( ( CMFCBoardClientDlg * )GetParent() )->SendDrawInfoToServer( m_ptStart, m_ptEnd );
			}
		}
	}
...
/// 清除图形
void CCanvasStatic::ClearGraphics()
{
/**************	Draw ***************/
	CPen pen;
	pen.CreatePen( PS_SOLID, 1, m_clDrawColor );
	CPen *pOldPen = m_dcMem.SelectObject( &pen );
	CBrush br( RGB(255,255,255) );
	CBrush *pOldBr = m_dcMem.SelectObject( &br );
	m_ptStart.x = m_ptStart.y = 0-2 ;
	m_ptEnd.x = m_ptEnd.y = 65536;
	CRect rect( m_ptStart, m_ptEnd );
	m_dcMem.Rectangle( rect );
	pOldPen->DeleteObject();
	pOldBr->DeleteObject();
	m_dcOld.BitBlt( 0, 0, GetSystemMetrics( SM_CXSCREEN ), 
		GetSystemMetrics( SM_CYSCREEN ),
		&m_dcMem, 0, 0, SRCCOPY );
	UpdataCanvas();
/**************	DRAWINFO drinfo;***********/
	//DRAWINFO drinfo;
	//drinfo.color = RGB(255,255,255);
	//drinfo.nWidth = 1;
	//drinfo.tool = RECTANGLE;
	//drinfo.ptStart.x = drinfo.ptStart.y = 0-2 ;
	//drinfo.ptEnd.x = drinfo.ptEnd.y = 65536 ;
	//Draw( drinfo );
}
void CCanvasStatic::ClearShape()
{
    ClearGraphics();
}
