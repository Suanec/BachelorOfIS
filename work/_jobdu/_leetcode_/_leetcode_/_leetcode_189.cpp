...
Total Accepted: 70034 Total Submissions: 331522 Difficulty: Easy
...
public:
  ...
  //      int i = k;
		//vector<int> tem ;
		//while( i > 0 ) tem.push_back( nums[ size - i-- ] );
		//i = 0;
		//int target = size - k - 1;
		//while( i <= target ) tem.push_back( nums[i++] );
		//i = 0;
		//while( i < size ) {
		//	nums[i] = tem[i] ;
		//	i++;
  ...
  //      int i = k;
		...
		//i = -1;
		//int target = size - k - 1;
		//while( ++i <= target ) nums[size-i-1] = nums[target-i];
		//i = 0;
		//while( i < k ) {
		//	nums[i] = tem[i] ;
		//	i++;
  ...
  //  }
	...
	void rotate(vector<int>& nums, int k) {
		...
		int cur = 0, cnt = 0, numStored = nums[0], tem = 0, start = 0, target = 0;
		while( cnt < size ){
			do{
				target = ( cur + k ) % size;
				tem = nums[target];
				nums[target] = numStored;
				numStored = tem;
				cur = target;
				cnt ++;
			}while( cur != start ); 
